function p=schnakinit(p,dom,mp,par)
    ...
    p.plot.auxdict={'a','b','c','D','L','||u_1||_{\infty}','min(|u_1|)'};
    ...
    b=par(2); % setting parameter Length
    u=b*zeros(p.np,1); % initial guess for u resp. u_1
    v=(1/b)*ones(p.np,1); % initial guess for v resp. u_2
